...
- API ハンドラーは HTTP 入出力（`req/res`）とメソッド分岐に寄せて薄く保つ
- DB 操作を `src/lib/queries.js` に集約（例：`getTodos / addTodo / deleteTodo / updateTodo`）
...
- 保守性：DB 接続やクエリの詳細を API から隠蔽（カプセル化）できている
...
**課題**
入力チェックが弱いと不正データが DB に混入し、後続処理で例外や不整合を招きやすい。

**打ち手**

**課題**  
入力チェックが弱いと不正データが DB に混入し、後続処理で例外や不整合を招きやすい。フロントと API でルールが分散すると、仕様変更時に更新漏れが起きて整合性が崩れやすい。

**打ち手（方法／採用理由）**

- API の入口で必ず検証し、サーバ側を最終防衛線として位置付け
- POST/PUT/DELETE などメソッドごとに必要な検証を適用（例：作成時の必須入力、PUT の完全置換、削除時の ID 形式など）
...
### 4) 開発環境のコンテナ化（チーム開発の標準化）

**課題**  
チーム開発において、開発者ごとに OS や Node.js のバージョンが異なると、「ローカルでは動くが他人の環境や CI で動かない」というトラブルが頻発する。また、新規メンバー参画時に環境構築だけで時間を消費してしまう（オンボーディングコストが高い）。

**打ち手（方法／採用理由）**

- 環境のコード化（IaC）: Dockerfile / docker-compose.yml により、OS レベルの依存関係を含めて環境定義をコード管理し、誰でも同じ環境を再現可能にする。
- マルチステージビルド: dev（開発）、ci（テスト）、runner（本番）を分離し、開発時の利便性（ホットリロード）と本番の軽量・安全性を両立。
- ワンコマンド構築: DB 接続設定や依存関係解決を docker-compose up 一発で完結させる構成。

**成果**

- チーム開発の効率化: OS（Mac/Windows）の差異をコンテナで吸収し、「環境依存のバグ」に関するコミュニケーションコストをゼロにした。
- 即戦力化: 新規メンバーはリポジトリをクローンしてコマンドを打つだけで、即座に開発に着手できる状態を実現した。
- 品質の担保: ローカルと CI で同一の Docker イメージを使用するため、テストの信頼性が向上した。
